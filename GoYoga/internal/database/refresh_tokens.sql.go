...
const queryUserIDFromToken = `-- name: QueryUserIDFromToken :one
SELECT user_id FROM refresh_tokens WHERE token = $1
`

func (q *Queries) QueryUserIDFromToken(ctx context.Context, token string) (uuid.UUID, error) {
	row := q.db.QueryRowContext(ctx, queryUserIDFromToken, token)
